/*
  ...
  1. New Tables
    ...
    - message_tags (user mentions, action items, follow-ups)
    - message_attachments (file uploads)
    - action_items (task management)

  2. Enhanced Tables
    - community_posts (add action item fields)

  3. Storage
    - message-attachments bucket for file uploads
  ...
  5. Functions & Triggers
    - Sync between action items and message tags
*/

-- Add enhanced fields to community_posts
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'community_posts' AND column_name = 'is_action_item') THEN
  ...
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'community_posts' AND column_name = 'due_date') THEN
    ALTER TABLE community_posts ADD COLUMN due_date timestamptz;
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'community_posts' AND column_name = 'assigned_to') THEN
    ALTER TABLE community_posts ADD COLUMN assigned_to uuid REFERENCES auth.users(id);
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'community_posts' AND column_name = 'metadata') THEN
...
DO $$
BEGIN
  -- Drop message_reactions policies
  ...
  -- Drop message_attachments policies
...
BEGIN
  ...
  DROP POLICY IF EXISTS "Community members can view message attachments" ON storage.objects;
...
DROP FUNCTION IF EXISTS create_message_tag_for_action_item() CASCADE;
DROP FUNCTION IF EXISTS sync_action_item_status() CASCADE;
