/*
  ...
  1. New Columns
    ...
    - `age_range` - User's age range for demographic targeting
    - `location` - User's location for regional communities
    - `experience_level` - User's experience level (beginner, intermediate, advanced, expert)
    - `preferences` - JSON object for user preferences (notifications, privacy, etc.)

  2. Changes
    - Rename `fitness_goals` to `interests` to be more generic
    - Remove `children_info` as it's specific to mom-focused platform
    - Add more generic profile fields for broader user base

  3. Data Migration
    - Preserve existing data by migrating fitness_goals to interests
