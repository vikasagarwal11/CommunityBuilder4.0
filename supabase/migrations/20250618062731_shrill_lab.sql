/*
  # Fix Events and Engagement Features

  1. New Tables
    - `events` - For community events
    - `admin_notifications` - For notifying admins about important events
    - `message_intents` - For detecting user intents in messages
  
  2. Functions
    - `increment_engagement` - For updating post engagement levels
    - `update_post_engagement_on_reaction` - Trigger function for reactions
    - `update_post_engagement_on_comment` - Trigger function for comments
  
  3. Policies
    - Added policies for all new tables
...
BEGIN
  IF NOT EXISTS (
    ...
    WHERE polname = 'Community members can view events' 
  ...
  IF NOT EXISTS (
    SELECT 1 FROM pg_policy 
    WHERE polname = 'Community admins can create events' 
    AND polrelid = 'events'::regclass
  ) THEN
  ...
  IF NOT EXISTS (
    SELECT 1 FROM pg_policy 
    WHERE polname = 'Community admins can update events' 
    AND polrelid = 'events'::regclass
  ) THEN
  ...
  IF NOT EXISTS (
    SELECT 1 FROM pg_policy 
    WHERE polname = 'Community admins can delete events' 
    AND polrelid = 'events'::regclass
  ) THEN
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policy 
    WHERE polname = 'Community admins can view notifications' 
    AND polrelid = 'admin_notifications'::regclass
  ) THEN
  ...
  IF NOT EXISTS (
    SELECT 1 FROM pg_policy 
    WHERE polname = 'Users can create notifications' 
    AND polrelid = 'admin_notifications'::regclass
  ) THEN
  ...
  IF NOT EXISTS (
    SELECT 1 FROM pg_policy 
    WHERE polname = 'Admins can update notifications' 
    AND polrelid = 'admin_notifications'::regclass
  ) THEN
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policy 
    WHERE polname = 'Community members can view message intents' 
    AND polrelid = 'message_intents'::regclass
  ) THEN
  ...
  IF NOT EXISTS (
    SELECT 1 FROM pg_policy 
    WHERE polname = 'Community admins can update message intents' 
    AND polrelid = 'message_intents'::regclass
  ) THEN
  ...
  IF NOT EXISTS (
    SELECT 1 FROM pg_policy 
    WHERE polname = 'Users can create message intents' 
    AND polrelid = 'message_intents'::regclass
  ) THEN
