-- Force clean database - drop all existing tables and start fresh
...
DROP TABLE IF EXISTS public.community_analytics CASCADE;
...
DROP FUNCTION IF EXISTS public.assign_community_admin() CASCADE;
DROP FUNCTION IF EXISTS public.notify_admins_of_flag() CASCADE;
DROP FUNCTION IF EXISTS public.update_engagement_score() CASCADE;
DROP FUNCTION IF EXISTS public.validate_role_assignment() CASCADE;
DROP FUNCTION IF EXISTS public.create_activity_on_post() CASCADE;

-- Drop any remaining triggers
DROP TRIGGER IF EXISTS on_auth_user_created ON auth.users;
DROP TRIGGER IF EXISTS assign_community_admin_trigger ON public.communities;
...
COMMENT ON SCHEMA public IS 'MomFit application schema - completely clean setup';