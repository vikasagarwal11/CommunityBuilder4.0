...
BEGIN
  -- Check if priority column exists in message_tags
  SELECT EXISTS (
    SELECT 1 FROM information_schema.columns 
    WHERE table_name = 'message_tags' 
  ...
  -- Test the message_tags query structure (with conditional priority column)
  IF column_exists THEN
    FOR test_result IN
      SELECT mt.id, mt.tag_type, mt.status, mt.due_date, mt.notes,
      ...
      LIMIT 1
    LOOP
      RAISE LOG 'Message tags query test successful (with priority)';
      EXIT;
    END LOOP;
  ELSE
    FOR test_result IN
      ...
      LIMIT 1
    LOOP
      RAISE LOG 'Message tags query test successful (without priority)';
      EXIT;
    END LOOP;
  END IF;
