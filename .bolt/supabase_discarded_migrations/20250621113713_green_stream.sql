/*
  # Enable RLS and add policies for post_comments table
  
  1. Security
    - Enable Row Level Security on post_comments table
    - Add policies for users to comment on posts in their communities
    - Add policies for users to view comments on accessible posts
  
  2. Foreign Keys
    - Check and add foreign key constraint between post_comments and profiles if needed
*/

-- Enable Row Level Security on post_comments table
ALTER TABLE post_comments ENABLE ROW LEVEL SECURITY;

-- Create policy for users to comment on posts in their communities
CREATE POLICY "Users can comment on posts in their communities"
ON post_comments
FOR INSERT
WITH CHECK (
  (uid() = user_id)
  AND EXISTS (
    SELECT 1 FROM community_posts
    WHERE id = post_id
    AND community_id IN (
      SELECT community_id FROM community_members
      WHERE user_id = uid()
    )
  )
);

-- Create policy for users to see comments on accessible posts
CREATE POLICY "Users can see comments on accessible posts"
ON post_comments
FOR SELECT
USING (
  EXISTS (
    SELECT 1 FROM community_posts
    WHERE id = post_id
    AND community_id IN (
      SELECT community_id FROM community_members
      WHERE user_id = uid()
    )
  )
);

-- Check if foreign key constraint already exists before adding it
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint 
    WHERE conname = 'fk_post_comments_user_profiles'
    AND conrelid = 'post_comments'::regclass
  ) THEN
    ALTER TABLE post_comments 
    ADD CONSTRAINT fk_post_comments_user_profiles 
    FOREIGN KEY (user_id) REFERENCES profiles(id) ON DELETE CASCADE;
  END IF;
END $$;